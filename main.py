...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # Conv 1x1 from layer 7
    conv_of_7 = tf.layers.conv2d(
        vgg_layer7_out,
        num_classes, 
        1,  # kernel size
        padding='same',
    ...
    conv_of_4 = tf.layers.conv2d(
        vgg_layer4_out_scaled,
        num_classes, 
        1,  # kernel size
        padding='same',
        kernel_initializer=tf.random_normal_initializer(stddev=weights_initializer_stddev),
    ...
    # Skip layer
    skip_1 = tf.add(upsample_1, conv_of_4, name='skip_1')
    ...
    conv_of_3 = tf.layers.conv2d(
        vgg_layer3_out_scaled,
        num_classes, 
        1,  # kernel size
        padding='same',
        kernel_initializer=tf.random_normal_initializer(stddev=weights_initializer_stddev),
     ...
     # Skip layer
    skip_2 = tf.add(upsample_2, conv_of_3, name='skip_2')
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    # Create loss funnction.
    cross_entropy_loss_raw = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=correct_label))
    
    # Make regularization really take effect.
    reg_losses = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
    reg_constant = 1
    cross_entropy_loss = cross_entropy_loss_raw + reg_constant * sum(reg_losses)
    
    # Create optimizer. Using Adam for adaptive learning rate.
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    log_file = open(os.path.join(log_dir, 'model.log'), 'w')
    
    for epoch in range(epochs):
        log_msg = 'Epoch: {}: '.format(epoch + 1)
        print(log_msg)
...
def save_model(sess):
    ...
    ckpt_filepath = os.path.join(model_dir, 'model.ckpt')
    saver.save(sess, ckpt_filepath)

    # This will only save the graph but the variables will not be saved.
    # You have to freeze your model first.
    tf.train.write_graph(graph_or_graph_def=sess.graph_def, logdir=model_dir, name='model.pb', as_text=False)
...
def run():
    ...
    with tf.Session() as sess:
        ...
        # Save model checkpoint and pb to file
        save_model(sess)
